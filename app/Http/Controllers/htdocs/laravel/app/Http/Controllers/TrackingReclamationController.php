...
class TrackingReclamationController extends Controller
{
    ...
    public function fetchProfesseurReclamations(Request $request)
    {
        ...
        $professeur = $request->input('professeur');
            ...
            ->where('r.AnneeUniversitaire','like', '2023-2024')
            ->where('professeurs.id', 'like', $professeur)
            ->where('m.Semester', 'like', $semester)
            ->where('f.id', 'like', $filiere)
            ->groupBy('professeurs.id', 'professeurs.Nom', 'professeurs.Prenom', 'professeurs.Email');
        if ($statu == 'not_Valide') {
